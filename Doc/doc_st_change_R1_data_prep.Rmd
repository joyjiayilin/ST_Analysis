...
doc_st_change_dat4 <- read.xlsx("doc_st_change_dat4.xlsx") %>% clean_dept_column()
...
doc_st_change_dat5 <- doc_st_change_dat4 %>%
        group_by(UID) %>%
...
doc_st_change_dat5 <- year_1 %>%
  left_join(year_2, by = c("UID", "Dept_code")) %>%
  .[, c(names(.)[names(.) != "STEM.y"])] %>%
  rename(STEM = STEM.x)

write.xlsx(doc_st_change_dat5, "doc_st_change_dat5.xlsx", rowNames = FALSE)
...
doc_st_change_miss1_all <- summarize_missing_records(doc_st_change_dat4, year_1, 3, event_types, '1st year')

# Summarize All Records Year 2
doc_st_change_miss2_all <- summarize_missing_records(doc_st_change_dat4, year_2, 6, event_types, '1st 2 years')

# Summarize STEM Records Year 1
doc_st_change_stem_miss1 <- summarize_missing_records(filter(doc_st_change_dat4, STEM == 1), year_1, 3, event_types, '1st year')

# Summarize STEM Records Year 2
doc_st_change_stem_miss2 <- summarize_missing_records(filter(doc_st_change_dat4, STEM == 1), year_2, 6, event_types, '1st 2 years')

# Summarize Non-STEM Records Year 1
doc_st_change_non_stem_miss1 <- summarize_missing_records(filter(doc_st_change_dat4, STEM == 0), year_1, 3, event_types, '1st year')

# Summarize Non-STEM Records Year 2
doc_st_change_non_stem_miss2 <- summarize_missing_records(filter(doc_st_change_dat4, STEM == 0), year_2, 6, event_types, '1st 2 years')
...
writeData(wb, sheet = "miss1", x = doc_st_change_miss1_all)
writeData(wb, sheet = "miss2", x = doc_st_change_miss2_all)
writeData(wb, sheet = "miss1_s", x = doc_st_change_stem_miss1)
writeData(wb, sheet = "miss2_s", x = doc_st_change_stem_miss2)
writeData(wb, sheet = "miss1_n", x = doc_st_change_non_stem_miss1)
writeData(wb, sheet = "miss2_n", x = doc_st_change_non_stem_miss2)

# Save the workbook to a file
saveWorkbook(wb, file = paste0(dirname(getwd()), "/Shiny/Doc_ST_Change_Reasons for Missing Data.xlsx"), overwrite = TRUE)
...
for (i in 1:10) {
  ...
  doc_st_change_dat4_F <- doc_st_change_dat4 %>%
    group_by(UID) %>%
    filter(substr(ENR_TERM, 1, 1) == 'F')
  
  year_i <- doc_st_change_dat4_F %>%
    group_by(UID) %>%
    slice(2)
  
  # Filter to get missing UIDs for this iteration
  doc_st_change_dat4_missing_i <- filter(doc_st_change_dat4, !(UID %in% year_i$UID))
  
  # Get UIDs with <=3i and >3i terms for this iteration
  doc_st_change_dat4_missing_i_no_more_than_3i_terms <- doc_st_change_dat4_missing_i %>%
    group_by(UID) %>%
    summarise(c = n()) %>%
    filter(c <= 3 * i)
  
  doc_st_change_dat4_missing_i_more_than_3i_terms <- doc_st_change_dat4_missing_i %>%
    group_by(UID) %>%
    summarise(c = n()) %>%
    filter(c > 3 * i)
  
  # Update the separated_i column based on the conditions
  doc_st_change_dat4 <- doc_st_change_dat4 %>%
    mutate(!!paste0('separated_', i) := case_when(
      UID %in% doc_st_change_dat4_missing_i_no_more_than_3i_terms$UID ~ 1,
      UID %in% doc_st_change_dat4_missing_i_more_than_3i_terms$UID & nth(LOA, 3 * i) == 1 ~ 1,
      UID %in% doc_st_change_dat4_missing_i_more_than_3i_terms$UID & nth(LOA, 3 * i) == 0 ~ 0,
      TRUE ~ !!sym(paste0('separated_', i))
    ))
  
  # Un-group the data after the transformations
  doc_st_change_dat4 <- ungroup(doc_st_change_dat4)
}
...
# access_uid <- doc_st_change_dat4$UID[doc_st_change_dat4$Dept_code == '0001']
# access_declare_major <- doc_st_change_dat4[doc_st_change_dat4$UID %in% access_uid & doc_st_change_dat4$Dept_code != '0001', ]
# access_not_declare_major <- doc_st_change_dat4[!(doc_st_change_dat4$UID %in% access_declare_major$UID) & doc_st_change_dat4$Dept_code == '0001', ]
# 
# doc_st_change_dat4$Dept[doc_st_change_dat4$UID %in% access_declare_major$UID] <- 
#   access_declare_major$Dept[match(doc_st_change_dat4$UID[doc_st_change_dat4$UID %in% access_declare_major$UID], access_declare_major$UID)]
# doc_st_change_dat4$Dept_code[doc_st_change_dat4$UID %in% access_declare_major$UID] <- 
#   access_declare_major$Dept_code[match(doc_st_change_dat4$UID[doc_st_change_dat4$UID %in% access_declare_major$UID], access_declare_major$UID)]
...
doc_st_change_dat6 <- doc_st_change_dat4 %>%
  group_by(across(all_of(grouping_vars))) %>%
  summarise(count = n(), .groups = 'keep')

doc_st_change_dat6 <- doc_st_change_dat6[, !(names(doc_st_change_dat6) %in% "count")]

write.xlsx(doc_st_change_dat6, "doc_st_change_dat6.xlsx", rowNames = FALSE)
