...
packages <- c("tidyverse", "readxl", "openxlsx", "writexl", 
              "margins", "tibble")
...
doc_dat4 <- read.xlsx("doc_dat4.xlsx") %>% clean_dept_column()
...
write.xlsx(doc_dat5, "doc_dat5.xlsx", rowNames = FALSE)
...
doc_miss1_all <- summarize_missing_records(doc_dat4, year_1, 3, event_types, '1st year')

# Summarize All Records Year 2
doc_miss2_all <- summarize_missing_records(doc_dat4, year_2, 6, event_types, '1st 2 years')

# Summarize STEM Records Year 1
doc_stem_miss1 <- summarize_missing_records(filter(doc_dat4, STEM == 1), year_1, 3, event_types, '1st year')

# Summarize STEM Records Year 2
doc_stem_miss2 <- summarize_missing_records(filter(doc_dat4, STEM == 1), year_2, 6, event_types, '1st 2 years')

# Summarize Non-STEM Records Year 1
doc_non_stem_miss1 <- summarize_missing_records(filter(doc_dat4, STEM == 0), year_1, 3, event_types, '1st year')

# Summarize Non-STEM Records Year 2
doc_non_stem_miss2 <- summarize_missing_records(filter(doc_dat4, STEM == 0), year_2, 6, event_types, '1st 2 years')
...
writeData(wb, sheet = "miss1", x = doc_miss1_all)
writeData(wb, sheet = "miss2", x = doc_miss2_all)
writeData(wb, sheet = "miss1_s", x = doc_stem_miss1)
writeData(wb, sheet = "miss2_s", x = doc_stem_miss2)
writeData(wb, sheet = "miss1_n", x = doc_non_stem_miss1)
writeData(wb, sheet = "miss2_n", x = doc_non_stem_miss2)

# Save the workbook to a file
saveWorkbook(wb, file = paste0(dirname(getwd()), "/Shiny/Doc_Reasons for Missing Data.xlsx"), overwrite = TRUE)
...
for (i in 1:10) {
  ...
  doc_dat4_missing_i <- filter(doc_dat4, !(UID %in% year_i$UID))
  
  # Get UIDs with <=3i and >3i terms for this iteration
  doc_dat4_missing_i_no_more_than_3i_terms <- doc_dat4_missing_i %>%
    group_by(UID) %>%
    summarise(c = n()) %>%
    filter(c <= 3 * i)
  
  doc_dat4_missing_i_more_than_3i_terms <- doc_dat4_missing_i %>%
    group_by(UID) %>%
    summarise(c = n()) %>%
    filter(c > 3 * i)
  ...
  doc_dat4 <- doc_dat4 %>%
    mutate(!!paste0('separated_', i) := case_when(
      UID %in% doc_dat4_missing_i_no_more_than_3i_terms$UID ~ 1,
      UID %in% doc_dat4_missing_i_more_than_3i_terms$UID & nth(LOA, 3 * i) == 1 ~ 1,
      UID %in% doc_dat4_missing_i_more_than_3i_terms$UID & nth(LOA, 3 * i) == 0 ~ 0,
      TRUE ~ !!sym(paste0('separated_', i))
    ))
  
  # Un-group the data after the transformations
  doc_dat4 <- ungroup(doc_dat4)
}
...
# access_uid <- doc_dat4$UID[doc_dat4$Dept_code == '0001']
# access_declare_major <- doc_dat4[doc_dat4$UID %in% access_uid & doc_dat4$Dept_code != '0001', ]
# access_not_declare_major <- doc_dat4[!(doc_dat4$UID %in% access_declare_major$UID) & doc_dat4$Dept_code == '0001', ]
# 
# doc_dat4$Dept[doc_dat4$UID %in% access_declare_major$UID] <- 
#   access_declare_major$Dept[match(doc_dat4$UID[doc_dat4$UID %in% access_declare_major$UID], access_declare_major$UID)]
# doc_dat4$Dept_code[doc_dat4$UID %in% access_declare_major$UID] <- 
#   access_declare_major$Dept_code[match(doc_dat4$UID[doc_dat4$UID %in% access_declare_major$UID], access_declare_major$UID)]
...
doc_dat6 <- doc_dat6[, !(names(doc_dat6) %in% "count")]

write.xlsx(doc_dat6, "doc_dat6.xlsx", rowNames = FALSE)
