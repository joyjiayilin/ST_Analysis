...
mas_st_change_dat4 <- read.xlsx("mas_st_change_dat4.xlsx") %>% clean_dept_column()
...
mas_st_change_dat5 <- mas_st_change_dat4 %>%
        group_by(UID) %>%
...
mas_st_change_dat5 <- year_1 %>%
  left_join(year_2, by = c("UID", "Dept_code")) %>%
  .[, c(names(.)[names(.) != "STEM.y"])] %>%
  rename(STEM = STEM.x)

write.xlsx(mas_st_change_dat5, "mas_st_change_dat5.xlsx", rowNames = FALSE)
...
mas_st_change_miss1_all <- summarize_missing_records(mas_st_change_dat4, year_1, 3, event_types, '1st year')

# Summarize All Records Year 2
mas_st_change_miss2_all <- summarize_missing_records(mas_st_change_dat4, year_2, 6, event_types, '1st 2 years')

# Summarize STEM Records Year 1
mas_st_change_stem_miss1 <- summarize_missing_records(filter(mas_st_change_dat4, STEM == 1), year_1, 3, event_types, '1st year')

# Summarize STEM Records Year 2
mas_st_change_stem_miss2 <- summarize_missing_records(filter(mas_st_change_dat4, STEM == 1), year_2, 6, event_types, '1st 2 years')

# Summarize Non-STEM Records Year 1
mas_st_change_non_stem_miss1 <- summarize_missing_records(filter(mas_st_change_dat4, STEM == 0), year_1, 3, event_types, '1st year')

# Summarize Non-STEM Records Year 2
mas_st_change_non_stem_miss2 <- summarize_missing_records(filter(mas_st_change_dat4, STEM == 0), year_2, 6, event_types, '1st 2 years')
...
writeData(wb, sheet = "miss1", x = mas_st_change_miss1_all)
writeData(wb, sheet = "miss2", x = mas_st_change_miss2_all)
writeData(wb, sheet = "miss1_s", x = mas_st_change_stem_miss1)
writeData(wb, sheet = "miss2_s", x = mas_st_change_stem_miss2)
writeData(wb, sheet = "miss1_n", x = mas_st_change_non_stem_miss1)
writeData(wb, sheet = "miss2_n", x = mas_st_change_non_stem_miss2)

# Save the workbook to a file
saveWorkbook(wb, file = paste0(dirname(getwd()), "/Shiny/Mas_ST_Change_Reasons for Missing Data.xlsx"), overwrite = TRUE)
...
for (i in 1:10) {
  ...
  mas_st_change_dat4_F <- mas_st_change_dat4 %>%
    group_by(UID) %>%
    filter(substr(ENR_TERM, 1, 1) == 'F')
  
  year_i <- mas_st_change_dat4_F %>%
    group_by(UID) %>%
    slice(2)
  
  # Filter to get missing UIDs for this iteration
  mas_st_change_dat4_missing_i <- filter(mas_st_change_dat4, !(UID %in% year_i$UID))
  
  # Get UIDs with <=3i and >3i terms for this iteration
  mas_st_change_dat4_missing_i_no_more_than_3i_terms <- mas_st_change_dat4_missing_i %>%
    group_by(UID) %>%
    summarise(c = n()) %>%
    filter(c <= 3 * i)
  
  mas_st_change_dat4_missing_i_more_than_3i_terms <- mas_st_change_dat4_missing_i %>%
    group_by(UID) %>%
    summarise(c = n()) %>%
    filter(c > 3 * i)
  
  # Update the separated_i column based on the conditions
  mas_st_change_dat4 <- mas_st_change_dat4 %>%
    mutate(!!paste0('separated_', i) := case_when(
      UID %in% mas_st_change_dat4_missing_i_no_more_than_3i_terms$UID ~ 1,
      UID %in% mas_st_change_dat4_missing_i_more_than_3i_terms$UID & nth(LOA, 3 * i) == 1 ~ 1,
      UID %in% mas_st_change_dat4_missing_i_more_than_3i_terms$UID & nth(LOA, 3 * i) == 0 ~ 0,
      TRUE ~ !!sym(paste0('separated_', i))
    ))
  
  # Un-group the data after the transformations
  mas_st_change_dat4 <- ungroup(mas_st_change_dat4)
}
...
# access_uid <- mas_st_change_dat4$UID[mas_st_change_dat4$Dept_code == '0001']
# access_declare_major <- mas_st_change_dat4[mas_st_change_dat4$UID %in% access_uid & mas_st_change_dat4$Dept_code != '0001', ]
# access_not_declare_major <- mas_st_change_dat4[!(mas_st_change_dat4$UID %in% access_declare_major$UID) & mas_st_change_dat4$Dept_code == '0001', ]
# 
# mas_st_change_dat4$Dept[mas_st_change_dat4$UID %in% access_declare_major$UID] <- 
#   access_declare_major$Dept[match(mas_st_change_dat4$UID[mas_st_change_dat4$UID %in% access_declare_major$UID], access_declare_major$UID)]
# mas_st_change_dat4$Dept_code[mas_st_change_dat4$UID %in% access_declare_major$UID] <- 
#   access_declare_major$Dept_code[match(mas_st_change_dat4$UID[mas_st_change_dat4$UID %in% access_declare_major$UID], access_declare_major$UID)]
...
mas_st_change_dat6 <- mas_st_change_dat4 %>%
  group_by(across(all_of(grouping_vars))) %>%
  summarise(count = n(), .groups = 'keep')

mas_st_change_dat6 <- mas_st_change_dat6[, !(names(mas_st_change_dat6) %in% "count")]

write.xlsx(mas_st_change_dat6, "mas_st_change_dat6.xlsx", rowNames = FALSE)
